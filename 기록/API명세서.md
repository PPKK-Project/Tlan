...
| 기능 | Method | 엔드포인트 | 설명 |
| :--- | :--- | :--- | :--- |
| **Google 장소 검색** | `GET` | `/place` | 키워드와 좌표를 기반으로 장소(숙소, 음식점 등)를 검색합니다. |
| **Google Geocoding** | `GET` | `/geocode` | 주소나 장소명을 위도/경도 좌표로 변환합니다. |
| **재외공관 정보 조회** | `GET` | `/embassy` | 국가 코드를 기반으로 재외공관(대사관) 정보를 조회합니다. |
| **환율 정보 조회** | `GET` | `/currency` | 실시간 환율 정보를 조회합니다. |
| **전 세계 여행경보 조회** | `GET` | `/countries` | 외교부의 여행경보 단계 정보를 조회합니다. |
